...
def evaluate_model(
    ...
    if CLASS_INDEX[args.obj] > 0:
        ...
        out["auc_img"] = float(auc_img)
        out["auc_pixel"] = float(auc_img)
        out["pauc"] = float(auc_img)
        out["combined"] = float(auc_img + auc_img)
    return out
...
def main():
    ...
    print(
        f"Checkpoint eval | AUC_img: {result['auc_img']:.4f} | AUC_pixel: {result['auc_pixel']:.4f} | "
        f"combined: {result['combined']:.4f}"
    )

    best = ckpt.get("best_metrics", None)
    if isinstance(best, dict):
        print(
            f"Checkpoint recorded best | epoch: {int(best.get('best_epoch', ckpt.get('epoch', -1)))} | "
            f"AUC_img: {float(best.get('auc_img', 0.0)):.4f} | "
            f"AUC_pixel: {float(best.get('auc_pixel', 0.0)):.4f} | "
            f"combined: {float(best.get('combined', 0.0)):.4f}"
        )
