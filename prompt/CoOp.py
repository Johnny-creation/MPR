...
class TextEncoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        width = self.ln_final.weight.shape[0]
        if self.text_adapt_until > 0:
            self.text_adapters = nn.ModuleList(
                [SimpleAdapter(width, width) for _ in range(self.text_adapt_until)]
            )
        else:
            self.text_adapters = nn.ModuleList()

        if self.text_projection is not None:
            self.text_projection.requires_grad_(bool(text_proj_trainable))
    ...
    def forward(self, prompts, tokenized_prompts):
        ...
        for i, block in enumerate(self.transformer.resblocks):
            x, _ = block(x, attn_mask=None)
            if i < self.text_adapt_until:
                adapt_out = self.text_adapters[i](x)
    ...
    def trainable_parameters(self):
        params = list(self.text_adapters.parameters())
        if self.text_projection is not None and self.text_projection.requires_grad:
...
class PromptLearner(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        model_device = clip_model.token_embedding.weight.device
        ...
        for cls in prompts:
            for position in class_token_position:
                if CSC:
                    ctx_vectors = torch.empty(len(prompts[cls]), n_ctx, ctx_dim, device=model_device) # [5,8,768]
                else:
                    ctx_vectors = torch.empty(n_ctx, ctx_dim, device=model_device)
        ...
        tokenized_prompts = {
            cls: torch.cat([tokenize(prompt) for prompt in prompts_learnable_tokens[cls]]).to(model_device)
