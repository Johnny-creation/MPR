...
class MedDataset(Dataset):
    ...
    def get_few_normal(self):
        ...
        if self.iterate < 0:
            ...
            random_choice = []
            seed_file = self._fewshot_seed_file()
            with open(seed_file, 'r', encoding='utf-8') as infile:
                for line in infile:
    ...
    def _fewshot_seed_file(self):
        seed_file = f'./dataset/fewshot_seed/{self.class_name}/{self.shot}-shot.txt'
        if os.path.exists(seed_file):
            return seed_file
        raise FileNotFoundError(
            f"No few-shot seed file found: {seed_file}"
        )
